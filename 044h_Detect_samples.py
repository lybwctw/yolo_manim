# ************************************************************
# Detect samples from yolov8 series for 3 classes.
# ************************************************************
from manim import *
import csv
...
with open(Path(__file__).with_name('yolov8_Detect_3.csv'), newline='') as csv_file:
    args = [
        f"{row['ch']} {row['c2']} {row['c3']} {row['reg_max']} {row['nc']}"
...
class MainScene(ThreeDScene):
    def construct(self):
        ...
        # ************************************************************
        # load card and graph
        card_ref = import_mobs('044g')
        cards = VGroup(card_ref.copy() for _ in range(13))
