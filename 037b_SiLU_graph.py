...
class MainScene(ThreeDScene):
    def construct(self):
        ...
        formula = MathTex(
            r"\operatorname{SiLU}(x) &= x \cdot \sigma(x) \\"
                r"&= \frac{x}{1 + e^{-x}}",
        ...
        dot = Dot(
            axes.c2p(x_tracker.get_value(), silu(x_tracker.get_value())),
            color=YELLOW,
            radius=0.04,
        )
        dot.add_updater(
        ...
        # ************************************************************
        self.next_section(
            'value map for silu function',
            skip_animations=False,
        )
        # ************************************************************
        def update_mapping(_):
            x_value.set_value(x_tracker.get_value())
            y_value.set_value(silu(x_tracker.get_value()))
