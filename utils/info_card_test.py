...
class InfoCard(VMobject):
    def __init__(
        ...
        if params is not None:
            self.name_objs, self.value_objs = self.create_objs()     # using ignores
            self.line_mobs = self.create_mobs()
    ...
    def create_objs(
        ...
        """Positioning based on self.head_mob.
        """
        mobs_name = {}
        mobs_value = {}
        ...
        for name, value in self.params.items():
            ...
            config = self.common_config if name in self.ignores else self.ignore_config
            mob_name = Text(name+':', **config)
            mob_value = Text(value, **config).next_to(
                mob_name[-1],
                buff=0.1,
            ).align_to(
                mob_name[-1],
            ...
            mobs_name[name] = mob_name
            mobs_value[name] = mob_value
        
        # align lines
        for idx, name in enumerate(self.params):
            line = VGroup(mobs_name[name], mobs_value[name])
            line.next_to(
                self.head_mob,
                DOWN,
                buff=0.2,
            ...
            line.shift(DOWN*0.3*idx)

        return mobs_name, mobs_value
